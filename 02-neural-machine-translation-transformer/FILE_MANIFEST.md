# Project 02 File Manifest

- `.dockerignore`
- `.env.example`
- `.gitignore`
- `CHANGELOG_FROM_ORIGINAL.md`
- `DEPLOYMENT_HUGGINGFACE.md`
- `DEPLOYMENT_HUGGINGFACE_STATIC.md`
- `Dockerfile`
- `MODEL_CARD.md`
- `PORTFOLIO_DEPLOYMENT_MAP.md`
- `README.md`
- `README_HUGGINGFACE.md`
- `STATIC_UPGRADE_INSTALLATION.md`
- `VALIDATION_REPORT_STATIC.md`
- `app.py`
- `configs/model_config.yaml`
- `data/README_data.md`
- `data/sample_batch_translation.csv`
- `data/sample_translation_pairs.csv`
- `gradio_app.py`
- `images/README.md`
- `models/README.md`
- `models/model_metadata.json`
- `notebooks/README.md`
- `notebooks/archive/00-original-provided-en-fr-baseline.ipynb`
- `notebooks/archive/README.md`
- `notebooks/neural_machine_translation_transformer.ipynb`
- `notebooks/translation_evaluation_and_error_analysis.ipynb`
- `outputs/README.md`
- `outputs/batch_translation_examples.csv`
- `outputs/chrf_scores.json`
- `outputs/manual_error_analysis.md`
- `outputs/model_comparison.csv`
- `outputs/model_metrics.json`
- `outputs/sacrebleu_scores.json`
- `outputs/translation_examples.csv`
- `outputs/translation_latency_results.json`
- `pyproject.toml`
- `pytest.ini`
- `requirements-ci.txt`
- `requirements-dev.txt`
- `requirements.txt`
- `scripts/evaluate_model.py`
- `scripts/run_batch_translation.py`
- `scripts/run_gradio.py`
- `scripts/train_model.py`
- `src/__init__.py`
- `src/batch_translation.py`
- `src/confidence_scoring.py`
- `src/config.py`
- `src/data_preprocessing.py`
- `src/dataset_loader.py`
- `src/inference_pipeline.py`
- `src/language_detection.py`
- `src/model_evaluation.py`
- `src/model_training.py`
- `src/text_preprocessing.py`
- `src/translation_model.py`
- `src/translation_pipeline.py`
- `src/visualization.py`
- `tests/conftest.py`
- `tests/test_batch_translation.py`
- `tests/test_inference_pipeline.py`
- `tests/test_language_detection.py`
- `tests/test_text_preprocessing.py`
- `tests/test_translation_pipeline.py`
- `web/README.md`
- `web/assets/favicon.svg`
- `web/data/evaluation-results.json`
- `web/data/sample_batch_translation.csv`
- `web/index.html`
- `web/package.json`
- `web/src/confidence.js`
- `web/src/csv.js`
- `web/src/language-detection.js`
- `web/src/main.js`
- `web/src/styles.css`
- `web/src/translation.worker.js`
- `web/tests/confidence.test.js`
- `web/tests/csv.test.js`
- `web/tests/language-detection.test.js`
