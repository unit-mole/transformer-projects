...
The metrics below were produced by running `sentence-transformers/all-MiniLM-L6-v2` against the included 34-chunk corpus and eight labelled evaluation queries. The browser loads the same completed JSON artifacts from `web/data/`; values are not hard-coded into the cards.
...
| Top-5 average Python query latency | **4.22 ms** |
| Top-5 minimum / maximum latency | **3.76 ms / 5.17 ms** |
| Cosine false-positive review | **Pending manual review** |

Evaluation artifacts:

```text
outputs/model_metrics.json
outputs/recall_at_k_results.json
outputs/mrr_results.json
outputs/query_latency_results.json
outputs/cosine_similarity_analysis.json
```

The cosine-analysis card deliberately remains marked as pending until similarity distributions and false positives are reviewed. No numeric cosine-analysis result is invented.
...
python scripts/export_browser_data.py --processed-dir data/processed --outputs-dir outputs --web-data-dir web/data
```
