...
## QASPER fine-tuning and benchmark extension

This repository contains an optional project-owned fine-tuning stage that
continues from the published SQuAD checkpoint using the contiguous extractive
subset of QASPER. A model may be described as fine-tuned by this project only
after the complete notebook has run and the resulting checkpoint and training
metadata exist under `models/qasper-longformer/` and `outputs/training_summary.json`.

The benchmark compares:

1. BERT question answering truncated to its first 512-token input;
2. the published Longformer SQuAD checkpoint with overlapping full-document windows;
3. the project-owned QASPER-fine-tuned Longformer checkpoint, when available.
